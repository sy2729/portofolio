...
riot.tag2('media', '<video autoplay loop id="video-background" muted plays-inline each="{videoProject}" if="{vid === videoId}" ref="video"> <source riot-src="{videoSrc1}" type="video/mp4"> </video> <div class="backupBackground"></div> <div class="content" if="{vid === videoId}" each="{videoProject}"> <div class="text"> <h2>{title}</h2> <p class="description">{description}</p> </div> <iframe width="400px" height="225" riot-src="{youtubeID}" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen id="youtubeVideo"></iframe> </div> <button onclick="{change}" id="changeProject">The Next</button>', 'media #video-background,[data-is="media"] #video-background{ position: fixed; right: 0; bottom: 0; min-width: 100%; min-height: 100%; width: auto; height: auto; z-index: -100; -webkit-filter: blur(4px) !important; filter: blur(4px) !important; -webkit-animation: fadeinBlur 2s ease-in-out; animation: fadeinBlur 2s ease-in-out; } media #changeProject,[data-is="media"] #changeProject{ position: absolute; bottom: 5%; right: 5%; background: transparent; transition: all .3s; padding: 15px 50px; border: 1px solid #fff; color: #fff; } media #changeProject:hover,[data-is="media"] #changeProject:hover{ cursor: pointer; background: rgba(255, 255, 255, 0.3); box-shadow: 2px 2px 12px 5px rgba(255, 255, 255, 0.3); } media .content,[data-is="media"] .content{ display: flex; justify-content: space-around; flex-wrap: wrap; } media .text,[data-is="media"] .text{ flex-basis: 2; margin-top: 150px; min-width: 350px; color: #fff; box-sizing: border-box; } media .text h2,[data-is="media"] .text h2{ max-width: 350px; line-height: 1.5em; } media .description,[data-is="media"] .description{ max-width: 350px; font-weight: 100; font-size: 0.95em; line-height: 2em; } media iframe,[data-is="media"] iframe{ flex-basis: 1; margin-top: 200px; } media #youtubeVideo,[data-is="media"] #youtubeVideo{ position: relative; } media .backupBackground,[data-is="media"] .backupBackground{ position: absolute; top: 0; bottom: 0; left: 0; right: 0; background: transparent; }', '', function(opts) {
    this.videoId = 1;
